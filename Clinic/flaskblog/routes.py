...
from flaskblog.models import User, Detail, Medicine, Patient, Worklog, Admin, Announcement, Newpatient
...
def update_patient(patient_id):
    ...
    if form.validate_on_submit(): 
        ...
        patient.year = form.year.data
        patient.month = form.month.data
        patient.month = form.day.data
        ...
        form.year.data = patient.year
        form.month.data = patient.month
        form.day.data = patient.month
...
def doctor_information(name):   
    announcements = Announcement.query.order_by(Announcement.date_posted.desc())
    amount_patient = Newpatient.query.first() # total new patient
    ...
    time = timedelta(weeks = 4) # filter last 4 weeks
    four_weeks_ago = current_time - time # start the counter 
    ...
    filter_by_month_1 = len(list(dict.fromkeys(filter_by_month))) # remove duplicate then use len() to get total
    ...
    filter_by_today_1 = len(list(dict.fromkeys(filter_by_today))) # remove duplicate then use len() to get total
    #visits = Newpatient.query.filter(date.month == Newpatient).all()
    return render_template('file.html', user=user, mylist1=mylist1, amount_patient=amount_patient, 
    values=Worklog.query.filter_by(author=user), announcements=announcements,
